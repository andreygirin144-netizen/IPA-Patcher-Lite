...
def _inject_load_dylib_into_slice(data, offset, dylib_install_name):
    ...
    for _ in range(ncmds):
        if cmd_offset + 8 > offset + header_size + sizeofcmds: break
    ...
    if available_space < len(new_cmd):
        log.error("Недостаточно места: нужно %d байт, доступно %d", len(new_cmd), available_space)
        return False
...
def _inject_rpath_into_slice(data, offset, rpath_path):
    ...
    for _ in range(ncmds):
        ...
        if cmd == LC_RPATH:
            ...
            if existing == rpath_path:
                log.info("LC_RPATH уже есть: %s", rpath_path)
                return True
        if cmdsize < 8: break
        cmd_offset += cmdsize
    ...
    available_space = min_section_offset - insert_at
    if available_space < len(new_cmd):
        log.error("Недостаточно места для LC_RPATH: нужно %d байт", len(new_cmd))
        return False
    ...
    struct.pack_into(endian + 'I', data, offset + 16, new_ncmds)
    struct.pack_into(endian + 'I', data, offset + 20, new_sizeofcmds)
    log.info("LC_RPATH добавлен: %s", rpath_path)
    return True
...
def inject_rpath(binary_path, rpath_path):
    ...
    if magic == FAT_MAGIC:
        ...
        for i in range(nfat):
            ...
            slice_offset = struct.unpack_from('>I', data, arch_off + 8)[0]
            if cputype in (0x0100000C, 12):
                if _inject_rpath_into_slice(data, slice_offset, rpath_path):
                    any_ok = True
        success = any_ok
    else:
...
def _check_encryption_in_slice(f, offset, big_endian=False):
    ...
    try:
        f.seek(offset)
        magic_bytes = f.read(4)
        if len(magic_bytes) < 4: return None
        magic = struct.unpack('<I', magic_bytes)[0]
        if magic == MH_CIGAM_64: endian = '>'
        elif magic == MH_CIGAM_32: endian = '>'
        header = f.read(24)
        if len(header) < 24: return None
        ncmds = struct.unpack(endian + 'I', header[12:16])[0]
        for _ in range(ncmds):
            cmd_data = f.read(8)
            if len(cmd_data) < 8: break
    ...
    if not os.path.isfile(executable_path):
        for root, _, files in os.walk(app_dir):
            if executable_name in files:
                ...
                break
    if not os.path.isfile(executable_path): return None
    try:
