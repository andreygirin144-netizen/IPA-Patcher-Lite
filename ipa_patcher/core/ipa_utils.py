...
def pick_ipa_file():
    ...
    if found:
        ...
        while True:
            try:
                ch = input("Выберите файл: ").strip()
                if ch == '0' or not ch:
                    break
                num = int(ch)
                if 1 <= num <= len(found):
                    return found[num-1][1]
                print("Неверный номер.")
            except ValueError:
                print("Введите число.")
    else:
        print(f"В {get_platform_temp_dir()} не найдено .ipa файлов.")

    path = input("Введите полный путь к .ipa: ").strip().strip('"')
...
def pick_tweak_file():
    ...
    if found:
        ...
        print(f"В {get_platform_temp_dir()} не найдено подходящих файлов твиков.")

    path = input("Введите полный путь к твику/архиву: ").strip().strip('"')
    return os.path.expanduser(path) if path else None
...
def pick_icon_file():
    ...
    if found:
        ...
        print(f"В {get_platform_temp_dir()} не найдено PNG файлов.")

    path = input("Введите полный путь к PNG: ").strip().strip('"')
    return os.path.expanduser(path) if path else None
...
def pick_substrate_file():
    if PYTHONISTA:
        ...
        if path:
            if not path.lower().endswith('.dylib'):
                print("Ошибка: нужен .dylib файл.")
    ...
    if found:
        ...
        print(f"В {get_platform_temp_dir()} не найдено .dylib для субстрата.")

    path = input("Введите полный путь к libsubstrate.dylib: ").strip().strip('"')
    return os.path.expanduser(path) if path else None
...
def pick_cert_zip():
    ...
    if found:
        ...
        print(f"В {get_platform_temp_dir()} не найдено .zip архивов.")

    path = input("Введите полный путь к .zip архиву с сертификатом: ").strip().strip('"')
    return os.path.expanduser(path) if path else None
